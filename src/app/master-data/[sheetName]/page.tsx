...
export default function MasterDataCrudPage() {
  ...
  // Fetch lookup values for foreign key columns
  const fetchLookupValues = useCallback(
    async (columns: SheetColumn[]) => {
      const lookups: Record<string, string[]> = {};
      for (const col of columns) {
        if (col.lookupSheet && col.lookupColumn) {
          try {
            const res = await fetch(
              `/api/sheets/lookup?sheet=${col.lookupSheet}&column=${col.lookupColumn}`
            );
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              {schema?.hasPlantFilter && (
                <Badge variant="outline">Plant: {user.plantId}</Badge>
              )}
            </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {modalMode === "create" ? "Add New" : "Edit"} {sheetName}
            </DialogTitle>
            <DialogDescription>
